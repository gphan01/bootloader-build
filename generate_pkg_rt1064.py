...
def discover_paths(config : configparser.ConfigParser) -> dict:
        ...
        if os.path.isdir(entry_path):
            ...
            if os.path.isfile(cproject_path):
                ...
                if not os.path.isfile(dot_project_path):
                    print(f'ERROR (discover_paths) - .project not found at {dot_project_path}', file=sys.stderr)
                    sys.exit(1)
    ...
    if not os.path.isdir(results_path):
        print(f'ERROR (discover_paths) - path not found at {results_path}', file=sys.stderr)
        sys.exit(1)
...
def prepare_dot_project(paths: dict) -> bool:
    ...
    if count == 0:
        print('ERROR (prepare_dot_project) - <name> element not found in .project', file=sys.stderr)
        return False
...
def prepare_project(paths: dict) -> bool:
    ...
    if debug_cfg_idx == -1:
        print('ERROR (prepare_project) - Debug cconfiguration not found', file=sys.stderr)
        return False
    ...
    if symbols_idx == -1:
        print('ERROR (prepare_project) - Debug defined-symbols option not found', file=sys.stderr)
        return False

    option_close_idx = text.find('</option>', symbols_idx)
    if option_close_idx == -1:
        print('ERROR (prepare_project) - closing </option> after defined-symbols not found', file=sys.stderr)
        return False
    ...
    pattern_pf2 = r'[ \t]*&lt;memoryInstance[^/]*?id="PROGRAM_FLASH2"[^/]*?/&gt;&#13;\n?'
    new_text, pf2_count = re.subn(pattern_pf2, '', text)
    if pf2_count > 1:
        print(f'ERROR (prepare_project) - PROGRAM_FLASH2 matched {pf2_count} times, expected 0 or 1', file=sys.stderr)
        return False
...
def discover_eoc(device : str, paths : dict) -> None:
    ...
    if len(matches) == 1:
        ...
        print(f'ERROR (discover_eoc) - No {device_extension} file found at {debug_path}', file=sys.stderr)
        sys.exit(1)
    else:
        print(f'ERROR (discover_eoc) - Multiple {device_extension} files found at {debug_path}', file=sys.stderr)
        sys.exit(1)
...
def package_firmware(paths: dict, device: str, input_path: str) -> str | None:
    ...
    if not os.path.isfile(pkg_path):
        print(f'ERROR (package_firmmware) - Generated package file missing at {pkg_path}\n')
        return None
