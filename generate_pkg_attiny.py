...
def main():
    ...
    #   - required: -t/--target (choices from DEVICES)
    #   - required: -v/--version
    ...
    parser.add_argument('-t', '--target', required=True, choices=["io", "driver"])
    ...
    # TODO 5: discover paths (pass config and args.target)
...
def discover_paths(config: configparser.ConfigParser, target: str) -> dict:
    """
    Finds the attiny project directory for the given target, plus FwPkgUtility paths.
    ...
    if project_path is None:
        print(
            f"ERROR - No attiny project directory found for target: {target}",
            file=sys.stderr,
        )
        sys.exit(1)
...
def discover_hex(paths: dict) -> None:
    ...
    pattern = os.path.join(
        project_path, "dist", "offset", "production", "*.production.hex"
    )
    ...
    if matches_len == 1:
        ...
        print(
            f"ERROR (discover_hex) - No .production.hex file found at {pattern}",
            file=sys.stderr,
        )
        sys.exit(1)
    else:
...
def package_firmware(paths: dict, target: str, version: str, input_path: str) -> str | None:
    ...
    if not os.path.isfile(pkg_path):
        print(f'ERROR (package_firmware) - Generated package file missing at {pkg_path}', file=sys.stderr)
        return None
